...
# アプリケーションのインストール
# 0x8A150101 (-1978335975): No applicable upgrade found (already up-to-date)
# 0x8A150109 (-1978335095): Already installed
$wingetNoUpgradeCode = -1978335975
$wingetAlreadyInstalledCode = -1978335095
$failed = @()
foreach ($app in $applications) {
    Write-Host "Installing $app..."
    winget install --id $app --silent --accept-package-agreements --accept-source-agreements
    $code = $LASTEXITCODE
    if ($code -ne 0 -and $code -ne $wingetNoUpgradeCode -and $code -ne $wingetAlreadyInstalledCode) {
...
# irm | iex 経由実行時は $PSScriptRoot が空になるため GitHub から取得する
Write-Host "`n--- AutoHotkey スタートアップ登録 ---"
$startupDir = "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup"
$ahkDest = Join-Path $startupDir "keybindings.ahk"

if ($PSScriptRoot -and (Test-Path (Join-Path $PSScriptRoot "keybindings.ahk"))) {
    Copy-Item -Path (Join-Path $PSScriptRoot "keybindings.ahk") -Destination $ahkDest -Force
    Write-Host "keybindings.ahk をスタートアップフォルダに登録しました。"
} else {
    $ahkUrl = "https://raw.githubusercontent.com/bobstrange/dotfiles/main/setup/windows/keybindings.ahk"
    Write-Host "GitHub から keybindings.ahk をダウンロードしてスタートアップに登録します..."
    irm -Headers @{"Cache-Control"="no-cache"} $ahkUrl -OutFile $ahkDest
    Write-Host "keybindings.ahk をスタートアップフォルダに登録しました。"
}
Write-Host "  → $ahkDest"
...
$backupPath = if ($PSScriptRoot) { Join-Path $PSScriptRoot "scancode_backup.bin" } else { Join-Path $env:USERPROFILE "scancode_backup.bin" }
